	...
	return Promise.all(searchURL.map((l) => axios.get(l))).then(
		axios.spread(function (...res) {
			let jsonData = res.map((e) =>
				getXMLRecord(
					new DOMParser().parseFromString(e.data, 'text/html')
				)
			)
			let result = jsonData.reduce((acc, record) => {
				...
				xml_record.map((e) => {
					...
					let thumbnail = deepSearch(
						e,
						fieldByDatabase[database].thumbnail
					)[0]
					...
					acc.push({
						...
						urlTitle: deepSearch(
							e,
							fieldByDatabase[database].title
						)[0],
						database,
...
export const getAllMedia = (record, database, mediaType = 'image') => {
	...
	return array.map((e) =>
		e
			.replace(/\n/g, '')
			.toLowerCase()
			.replace('diefenbunkermuseum', 'cams-diefenbunker')
	)
}
...
export const removeAllBookmarkRecord = () => {
	document.cookie =
		'BOOKMARK=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;'
}
