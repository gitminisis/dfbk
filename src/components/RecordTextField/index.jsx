...
const index = (props) => {
	...
	return displayFields.map((field) => {
		...
		let sameLabelField = displayFields.filter(
			(e, i) => e.label === field.label
		)
		let fieldValue
		if (sameLabelField.length > 1) {
			displayFields
				.filter((e, i) => e.label === field.label)
				.filter((e, i) => i !== 0)
				.forEach((e) => {
					e.duplicate = true
				})
			fieldValue = sameLabelField.map(
				(e) => deepSearch(recordData, e.name.toLowerCase())[0]
			)
		} else {
			fieldValue =
