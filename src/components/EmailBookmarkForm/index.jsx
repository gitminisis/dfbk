					...
					onSubmit={(event) => {
						...
						body = body
							.map((record, i) => {
								...
								let database = deepSearch(
									record,
									'database_name'
								)[0]
								let permarLink = getEmailPermalink(
									sisn,
									database
								)
								let title = deepSearch(
									record,
									TITLE_BY_DATABASE[database]
								)[0]
						...
						sendEmailBookmark(session, formValue, body).then(
							(res) => {
								enqueueSnackbar(
									`Your bookmark(s) list has been sent.`
								)
								setOpen(false)
							}
						)
